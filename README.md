...
## 18) Добавленные MVP-артефакты (фактически в репозитории)

- `client/`:
  - `package.json`, `electron-main.ts`, `src/index.tsx`, страницы (`Login`, `Register`, `ServerList`, `ChannelView`, `FriendsView`, `VoiceRoom`), `src/rtc/client.ts`, `config.json.example`.
- `server/`:
  - `package.json`, `src/index.ts`, контроллеры `auth/friends/servers/messages/media`, `src/rtc/mediasoup.ts`, `src/ws/socket.ts`, `.env.example`.
- `db/`:
  - `schema.sql`, `seed.sql` (алиасы к `001_schema.sql`, `002_seed.sql`).
- `infra/`:
  - `nginx/nginx.conf`, `coturn/turnserver.conf`.
- Root:
  - `docker-compose.yml` и `server/Dockerfile`.
- Docs:
  - `docs/openapi.yaml`, `docs/ws-spec.md`.
- CI:
  - `.github/workflows/ci.yml`.

## 19) Быстрый запуск тестовой среды (MVP skeleton)

1. Настроить `server/.env` на основе `server/.env.example`.
2. Убедиться, что SQL-скрипты применены (`db/schema.sql`, `db/seed.sql`).
3. Запустить backend локально:
   - `cd server && npm install && npm run dev`
4. Запустить client локально:
   - `cd client && npm install && npm run dev`
5. Для docker-среды:
   - `docker compose up --build`

> Примечание: это MVP skeleton-код для старта разработки и прохождения базовых сценариев; production hardening и полный медиа-пайплайн дополняются на этапах V1/V2.
